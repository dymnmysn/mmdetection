...
class TwoStagePanopticSegmentor(TwoStageDetector):
    ...
    def __init__(
        ...
        if panoptic_fusion_head is not None:
            ...
            self.insemantic_head = build_head(dict(
                type = 'InSemanticHead'
            ))
    ...
    def forward_train(self,
        ...
        assert not sum([a.isnan().any() for a in x])
        losses = dict()
        """
        ...
        """
        compound_mask = kwargs['compound_mask']
        insemantic_loss = self.insemantic_head.forward_train(x,compound_mask)
        losses.update({'insemantic_loss':insemantic_loss})
